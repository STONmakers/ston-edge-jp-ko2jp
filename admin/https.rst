...
別のIPアドレスまたはポートを指定しない場合、デフォルトでバインドされているサービスアドレスは、 "*:443"である。 グローバル設定（server.xml）に設定する。 ::
...
サーバーでSSLを使用する仮想ホストが一つであれば問題にならない。 しかし、複数の仮想ホストを同時に運営するサーバーでは、問題になることができる。 なぜなら、サーバーがクライアントに証明書を転送するときに（"一般HTTPS通信"の "2.証明書伝達") クライアントがどのようなHostに接続しようとして知ることができないからである。
...
Multi Certificate	証明書のみを交換して動作                            メインドメインまたはサービス主体が同じこと
Multi Port          ポートマン変更して動作                             WebページからのHTTPSポートを明示ヘジュオヤする

...
基本的にはTTLは、元のサーバーの応答に基づいて決定される。 TTLが期限切れになるまで保存されたコンテンツにサービスされる。 TTLが期限切れになると、元のサーバーにコンテンツの変更かどうか( **If-Modified-Since** または **If-None-Match** )を確認する。 ソースサーバーが **304 Not Modified** 応答をくれればTTLは、延長される。 ::
